...
api.objectTracker.createDataSet("../resources/datasets/ArgonTutorial.xml").then( (dataSet)=>{
    ...
    dataSet.load().then(()=>{
        ...
        // about.
        const techSquareEntity = app.context.subscribeToEntityById(trackables["TechSquare"].id)
...
api.objectTracker.createDataSet("../resources/datasets/ArgonTutorial.xml").then(function (dataSet) {
    ...
    dataSet.load().then(function () {
        ...
        // entities, we can ask for their pose in any coordinate frame we know
        // about.
        var techSquareEntity = app.context.subscribeToEntityById(trackables["TechSquare"].id);
        // create a THREE object to put on the trackable
